...
% !! Terminate when come back to (0,0):
if norm(r - [p_x; p_y]) <= 0.01 && Pos_reached == true
    u = [0; 0];
    if plotted == false
        % Question 6: Plot:
        axes2       = axes('Parent',figure);
        %   1. Desired robot trajectory
        r_traj      = plot(axes2,r_mem(1,:), r_mem(2,:));
        hold on
